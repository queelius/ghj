...
class FilterQuery:
    ...
    def __init__(self, query: Union[List, Dict]):
        self.query = query
        self.ast = self.parse_query(query)

    def parse_query(self, query: Union[List, Dict]) -> Any:
        """Parse the query into an AST composed of Condition and LogicalOperation objects."""
        if isinstance(query, list):
            ...
            if isinstance(operator, str):
                ...
                if operator_lower in LogicalOperation.logical_operators:
                    operands = [self.parse_query(subquery) for subquery in query[1:]]
            ...
            parsed_operands = [self.parse_query(subquery) for subquery in operands]
...
def filter_repos(repos: List[Dict], query: Union[List, Dict]) -> List[Dict]:
    """
    Filter repositories based on custom conditions using an AST-based query.

    Args:
        repos: List of repository dictionaries.
        query: Nested list or dictionary representing the filter conditions and logical operators.
...
class DSLTransformer(Transformer):
    ...
    def condition(self, items):
        field, comparator, value = items
        return Condition(field=field, operator=comparator, value=value)
    ...
    def comparator(self, items):
        # Return the comparator as a string
        return str(items[0])
